			...
			try {
				...
				if (!Objects.isNull(message)) {
					if (DasChatUtil.beginningEquals(message, "register:")) {
						...
						if (tempPassword.equals(account.getPassword())) {
							...
							for (Channel channel : Channel.getChannels()) {
								if (channel.getUsers().contains(tempAccountName) || channel.isPublic()) {
									String channelString = "<data id='channel'/>";
									Document channelDocument = Jsoup.parse(channelString);
									Element channelElement = channelDocument.getElementById("channel");
									String users = "";
									for (String user : channel.getUsers()) {
										users = users + "," + user;
									}
									channelElement.attr("users", users);

									String admins = "";
									for (String admin : channel.getUsers()) {
										admins = admins + "," + admin;
									}
									channelElement.attr("admins", admins);

									channelElement.attr("public", channel.isPublic());

									channelElement.attr("name", channel.getName());

									System.out.println(channelElement.toString());
									communication.send(channelElement.toString());
								}
							}
						...
						String time = gregCalendar.get(Calendar.HOUR) + ":" + gregCalendar.get(Calendar.MINUTE) + ":"
								+ gregCalendar.get(Calendar.SECOND);

						String date = gregCalendar.get(Calendar.DAY_OF_MONTH) + "." + gregCalendar.get(Calendar.MONTH)
